...
#include <math.h>
#include <fstream>
#include "LR1CellItoSK.h"

#ifndef LR1CellItoSK_cpp
#define LR1CellItoSK_cpp
...
#define ibarca (5.4e-4)*ibarcafac
...
template <int ncells>
 LR1CellItoSK<ncells>::LR1CellItoSK()
{
...
template <int ncells>
  bool LR1CellItoSK<ncells>::iterate(const int id, double dt, double st, double dv_max) {
    double dm, dh, dj, dd, df, dzdv, dydv, dxr, dcai;
    double ina, ical, ito, ikr, ik1, ipk, ib, isk, dv;
    ...
    ib = comp_ib(id);
    isk = comp_isk(id);
    dv = (diffcurrent[id] - (ikr + ik1 + ito + ina + ical + ib + ipk + isk + st))*dt;
...
template <int ncells>
void LR1CellItoSK<ncells>::stepdt (const int id, double dt, double st) {
    if (id > -1 && id < ncells) {
...
template <int ncells>
  double LR1CellItoSK<ncells>::comp_ina (int id, double dt, double& dm, double& dh, double& dj) //Fast Sodium Current
{
...
/*
template <int ncells>
  double LR1CellItoSK<ncells>::comp_ical (int id, double dt, double& dd, double& df) // L-type Calcium Current
{
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_ical (int id, double dt, double& dd, double& df)
{
    double taud, tauf, dss, fss, vfrt, ical;
    taud = 0.29 + 1.1*exp(0.052*v[id] + 13)/(1 + exp(0.132*(v[id]+13)));
    ...
    if (fabs(vfrt) < 1e-3) {
        ical = ibarca*d[id]*f[id]*zca*frdy*(cai[id]*exp(vfrt) - 0.341*cao)/(1.0 + vfrt/2.0 + vfrt*vfrt/6.0 + vfrt*vfrt*vfrt/24.0);
    }
    else {
        ical = ibarca*d[id]*f[id]*zca*frdy*vfrt*(cai[id]*exp(vfrt) - 0.341*cao)/(exp(vfrt) - 1.0);
    }
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_ito (int id, double dt, double& dzdv, double& dydv)
{
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_isk (int id)
{
    double isk, ek, z;
    z = 1.0/(1.0 + pow(skh/cai[id],skn));
    ek = (rtf/zk)*log(ko/ki);
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_ikr (int id, double dt, double& dxr)
{
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_ik1 (int id)
{
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_ipk (int id)
{
    double ek, ipk, kp;
...
template <int ncells>
double LR1CellItoSK<ncells>::comp_ib (int id)
{
    return ibbar*(v[id] + 59.87);
}

template <int ncells>
void LR1CellItoSK<ncells>::comp_calcdyn (int id, double ical, double& dcai) // MAKE SURE THIS IS COMPUTED AFTER ALL OTHER CURRENTS
{
    dcai = -0.0001*ical + 0.07*(0.0001 - cai[id]);
}

template <int ncells>
void LR1CellItoSK<ncells>::setcell (int id, LR1CellItoSK<1>* newcell)
{
    v[id] = newcell->v[0];
...
template <int ncells>
void LR1CellItoSK<ncells>::getcell (int id, LR1CellItoSK<1>* newcell)
{
    newcell->v[0] = v[id];
...
template <int ncells>
void LR1CellItoSK<ncells>::saveconditions(FILE* file, int id, bool header, double t) {
    if (header) {
