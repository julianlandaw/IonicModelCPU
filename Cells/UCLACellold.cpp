...
UCLACell<ncells>::UCLACell()
{
    for (int i = 0; i < ncells; i++) {
        ...
        xr[i] = 0.008709989976;// ikr gate variable 
        xs1[i] = 0.08433669901;// iks gate variable
        xs2[i] = 0.1412866149;// iks gate varaible 
...
bool UCLACell<ncells>::iterate(const int id, double dt, double st, double dv_max) {
    ...
    dv = (diffcurrent[id] - (xina + xik1 + xikr + xiks + xito + xinaca + xica + xinak + st))*dt;
...
double UCLACell<ncells>::comp_ina (int id, double dt, double& dxm, double& dxh, double& dxj) //Fast Sodium Current
{
    ...
    xina = gna*xm[id]*xm[id]*xm[id]*xh[id]*xj[id]*(v[id]-ena);
    
    dxm = (mss-(mss-xm[id])*exp(-dt/taum) - xm[id])/dt;
    dxh = (hss-(hss-xh[id])*exp(-dt/tauh) - xh[id])/dt;
    dxj = (jss-(jss-xj[id])*exp(-dt/tauj) - xj[id])/dt;
...
double UCLACell<ncells>::comp_ikr (int id, double dt, double& dxr)
{
    ...
    xikr = gkr*gss*xr[id]*rg*(v[id] - ek);
    
    dxr = (xkrinf - (xkrinf - xr[id])*exp(-dt/taukr) - xr[id])/dt;
...
{
    double prnak, eks, xs1ss, xs2ss, tauxs1, tauxs2, gksx, xiks;
    prnak = 0.018330;
    ...
    xiks = gks*gksx*xs1[id]*xs2[id]*(v[id] - eks);
...
{
    double ek, gki, aki, bki, xkin, xik1;
...
double UCLACell<ncells>::comp_inak(int id) {
    double xkmko, xkmnai, sigma, fnak, xinak;
    xkmko = 1.5;
    xkmnai = 12.0;
...
double UCLACell<ncells>::comp_inaca(int id, double csm) {
    double zw3, zw4, xkdna, aloss, xmcao, xmnao, xmnai, xmcai, yz1, yz2, yz3, yz4, zw8, xinacaq;
    ...
    zw8 = yz1 + yz2 + yz3 + yz4;
    xinacaq = gnaca*aloss*zw3/(zw4*zw8);
    return xinacaq;
}
...
{
    double pca, za, factor1, factor, rxa;
    pca = 0.00054;
    za = v[id]*2.0*frt;
    factor1 = 4.0*pca*xf*xf/(xxr*temp);
    factor = v[id]*factor1;
    if (fabs(za) < 0.001) {
...
{
    double bcal, xkcal, srmax, srkd, bmem, kmem, bsar, ksar, bpx, spx, mempx, sarpx;
    bcal = 24.0;
    xkcal = 7.0;
    srmax = 47.0;
    srkd = 0.6;
