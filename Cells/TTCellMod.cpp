...
#ifndef TTCellMod_cpp
#define TTCellMod_cpp
...
template <int ncells>
bool TTCellMod<ncells>::iterate(const int id, double dt, double st, double dv_max) {
    double dm, dh, dj, dd, df, dfca, dz, dy, dxs, dxr1, dxr2, dg;
    double dcai, dcasr, dnai, dki, dv;
    double ina, ical, ito, iks, ikr, ik1, inaca, inak, ipca, ipk, ibna, ibca;
    
    ina = comp_ina(id, dt, dm, dh, dj);         // fast sodium current
    ical = comp_ical(id, dt, dd, df, dfca);     // window current
    ito = comp_ito(id, dt, dz, dy);         // UCLA Ito current
    ...
    dv = (diffcurrent[id] - (ikr + iks + ik1 + ito + ina + ibna + ical + ibca + inak + inaca + ipca + ipk + st))*dt;
    ...
    dki = -(ik1 + ito + ikr + iks - 2.0*inak + ipk + st - diffcurrent[id])/(vc*frdy)*capacitance;
...
template <int ncells>
void TTCellMod<ncells>::stepdt (const int id, double dt, double st) {
    if (id > -1 && id < ncells) {
...
template <int ncells>
double TTCellMod<ncells>::comp_ina (int id, double dt, double& dm, double& dh, double& dj) //Fast Sodium Current
{
...
/*
#define pca (5.4e-4)
// half of the LR2 value, used in Miyoshi
#define gcai 1.0
#define gcao 0.341
//#define kmca 0.6

template <int ncells>
double TTCellMod<ncells>::comp_ical (int id, double dt, double& dd, double& df, double& dfca) // L-type Calcium Current
{
    ...
    if (fabs(vfrt) < 1e-3) {
        ical = ibarcafac[id]*pca*d[id]*f[id]*fca[id]*zca*frdy*(gcai*cai[id]*exp(vfrt) - gcao*cao)/(1.0 + vfrt/2.0 + vfrt*vfrt/6.0 + vfrt*vfrt*vfrt/24.0);
    }
    else {
        ical = ibarcafac[id]*pca*d[id]*f[id]*fca[id]*zca*frdy*vfrt*(gcai*cai[id]*exp(vfrt) - gcao*cao)/(exp(vfrt) - 1.0);
    }
...
/*
template <int ncells>
double TTCellMod<ncells>::comp_ical (int id, double dt, double& dd, double& df) // L-type Calcium Current, Miyoshi
{
    ...
    if (fabs(vfrt) < 1e-3) {
        ical = ibarcafac[id]*pca*d[id]*f[id]*fca*zca*frdy*(gcai*cai[id]*exp(vfrt) - gcao*cao)/(1.0 + vfrt/2.0 + vfrt*vfrt/6.0 + vfrt*vfrt*vfrt/24.0);
    }
    else {
        ical = ibarcafac[id]*pca*d[id]*f[id]*fca*zca*frdy*vfrt*(gcai*cai[id]*exp(vfrt) - gcao*cao)/(exp(vfrt) - 1.0);
    }
...
template <int ncells>
double TTCellMod<ncells>::comp_ito (int id, double dt, double& dz, double& dy)
{
    double ek, zss, tauz, yss, tauy, ito;
    
    ek = (rtf/zk)*log(ko/ki[id]);
    
    zss = 1.0/(1.0 + exp(-(v[id] + 3.0)/15.0));
    tauz = 3.5*exp(-(v[id]/30.0)*(v[id]/30.0))+1.5;
    
    yss = 1.0/(1.0+exp((v[id] +33.5)/10.0));
    tauy = 20.0/(1.0+exp((v[id] +33.5)/10.0)) + 20.0;
    
    ito = itofac[id]*gito*z[id]*y[id]*(v[id]-ek);
    
    dz = (zss-(zss-z[id])*exp(-dt/tauz) - z[id])/dt;
...
template <int ncells>
double TTCellMod<ncells>::comp_iks (int id, double dt, double& dxs)
{
...
template <int ncells>
double TTCellMod<ncells>::comp_ikr (int id, double dt, double& dxr1, double& dxr2)
{
...
template <int ncells>
double TTCellMod<ncells>::comp_ik1 (int id)
{
...
template <int ncells>
double TTCellMod<ncells>::comp_inaca (int id)
{
    double inaca;
    inaca = nacafac[id]*knaca*(exp(gamma*v[id]*frt)*nai[id]*nai[id]*nai[id]*cao - exp((gamma-1.0)*v[id]*frt)*nao*nao*nao*cai[id]*alpha)/((kmnai*kmnai*kmnai + nao*nao*nao)*(kmca_ncx + cao)*(1.0 + ksat*exp((gamma-1.0)*v[id]*frt)));
    
    return inaca;
}

template <int ncells>
double TTCellMod<ncells>::comp_inak (int id)
{
    double inak;
...
template <int ncells>
double TTCellMod<ncells>::comp_ipca (int id)
{
    double ipca;
...
template <int ncells>
double TTCellMod<ncells>::comp_ipk (int id)
{
    double ipk, ek;
    ek = (rtf/zk)*log(ko/ki[id]);
...
template <int ncells>
double TTCellMod<ncells>::comp_ibna (int id)
{
    double ena, ibna;
    ena = (rtf/zna)*log(nao/nai[id]);
    ibna = gbna*(v[id] - ena);
...
template <int ncells>
double TTCellMod<ncells>::comp_ibca (int id)
{
    double eca, ibca;
...
template <int ncells>
void TTCellMod<ncells>::comp_calcdyn (int id, double dt, double ical, double ibca, double ipca, double inaca, double &dg, double &dcasr, double &dcai) // MAKE SURE THIS IS COMPUTED AFTER ALL OTHER CURRENTS
{
...
template <int ncells>
void TTCellMod<ncells>::setcell (int id, TTCellMod<1>* newcell)
{
    v[id] = newcell->v[0];
...
template <int ncells>
void TTCellMod<ncells>::getcell (int id, TTCellMod<1>* newcell)
{
    newcell->v[0] = v[id];
...
template <int ncells>
void TTCellMod<ncells>::saveconditions(FILE* file, int id, bool header, double t) {
    if (header) {
        fprintf(file,"t\tv\tm\th\tj\td\tf\tz\ty\txs\txr1\txr2\tg\tcai\tcasr\tnai\tki\n");
    }
    fprintf(file,"%g\t",t);
...
template <int ncells>
void TTCellMod<ncells>::write(std::fstream& file) {
    file.write((char*)&this, sizeof (this) );
}

template <int ncells>
void TTCellMod<ncells>::read(std::fstream& file) {
