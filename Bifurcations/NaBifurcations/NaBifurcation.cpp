...
#include "NaBifurcation.h"

#ifndef NaBifurcation_cpp
#define NaBifurcation_cpp
...
NaBifurcation<typecell, ncells, beats>::NaBifurcation() {
    int i;
...
template <template<int> class typecell, int ncells, int beats>
void NaBifurcation<typecell, ncells, beats>::iterate(const int id, long double dt, long double t) {
    if (id < ncells && curbeat[id] < beats) {
        ...
        #define NAPAUSE 100            
            if (curbeat[id] >= REMOVEBEATS) {
                Cells.nai[id] = startna[id] + 1.0*NAPAUSE*((curbeat[id] - REMOVEBEATS)/NAPAUSE)*(endna[id] - startna[id])/(BEATS - REMOVEBEATS);
            }
            startapds[id] = t;
...
template <template<int> class typecell, int ncells, int beats>
void NaBifurcation<typecell, ncells, beats>::iterateall(long double dt, long double t) {
...
template <template<int> class typecell, int ncells, int beats>
void NaBifurcation<typecell, ncells, beats>::dobif(long double dt, double startt) {
