...
#include "DIBifurcation.h"

#ifndef DIBifurcation_cpp
#define DIBifurcation_cpp
...
DIBifurcation<typecell, ncells, beats>::DIBifurcation() {
    int i;
...
template <template<int> class typecell, int ncells, int beats>
void DIBifurcation<typecell, ncells, beats>::iterate(const int id, long double dt, long double t) {
    if (id < ncells && curbeat[id] < beats - 1) {
        double vold = Cells.v[id];
        st[id] = ((t > -dt/4.0L) && (t > t_endap[id] + dis[id] - dt/4.0L) && (t < t_endap[id] + dis[id] + stimduration - dt/4.0L) && (vold < threshold || st[id] < -1.0)) ? stimulus : 0.0;
#ifdef TTMod
        Cells.ki[id] = 140.0;
        Cells.nai[id] = 10.0;
#endif
...
template <template<int> class typecell, int ncells, int beats>
void DIBifurcation<typecell, ncells, beats>::iterateall(long double dt, long double t) {
...
template <template<int> class typecell, int ncells, int beats>
void DIBifurcation<typecell, ncells, beats>::dobif(long double dt, long double startt) {
